 ...
 */
trait WpTravelDeprecatedClassTrait {
	/**
	...
	private static function deprecated_class( $version, $replacement = null ) {
		 ...
		 */
		do_action( 'wptravel_deprecated_class_run', static::class, $version, $replacement );
		if ( ! WP_DEBUG ) {
			return;
		}
		/**
		 ...
		 */
		if ( ! apply_filters( 'wptravel_deprecated_class_trigger_error', true ) ) {
			return;
		}
		if ( function_exists( '__' ) ) {
			if ( ! empty( $replacement ) ) {
				...
				$message = sprintf(
					...
					__( 'The called class %1$s is <strong>deprecated</strong> since version %2$s! Use %3$s instead.', 'wp-travel' ),
				...
				$message = sprintf(
					...
					__( 'The called class %1$s is <strong>deprecated</strong> since version %2$s!', 'wp-travel' ),
					'<code>' . static::class . '</code>',
					'<strong>' . $version . '</strong>'
				);
			}
